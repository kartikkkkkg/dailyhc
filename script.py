...
# ==============================================
# CONFIG – EDIT THESE PATHS / SHEET NAMES
# ==============================================

# Input files
CURRENT_HC_PATH = Path("hc_current.xlsx")      # today's HC (Tech & Ops)
LAST_HC_PATH    = Path("hc_last_month.xlsx")   # last month-end HC
MAPPING_WB_PATH = Path("mapping.xlsx")         # has sheets: mapping, rollup

MAPPING_SHEET_NAME = "mapping"
ROLLUP_SHEET_NAME  = "rollup"

# Output file
OUTPUT_PATH       = Path("HC_Summary_CONTROLLABLE.xlsx")
...
# Ordering of Cost Cats for summary
...
def get_date_labels():
    """Return (current_label, last_month_end_label) as strings like '2-Dec'."""
    today = date.today()
    ...
    # On Windows, %-d may not work; fall back:
...
def classify_movements(combined: pd.DataFrame, rollup_ids: set) -> pd.DataFrame:
    ...
    Add ContrSide flag for later filtering (Controllable vs Non-Controllable views).
    """
    rows = []
        ...
        rows.append(
            {
                ...
                "FTE": float(fte),
                "EmpType": emp_type,
                "ContrSide": contr_side,   # Controllable / Non-Controllable
    ...
    for _, r in combined.iterrows():
        ...
        # In both months
            ...
            if same_mtcc:
                ...
                # Transfer out (side = last month status)
                add_move(
                    bid,
                    mt_cc_last,
                    cost_last,
                ...
                # Transfer in (side = current status)
                add_move(
                    bid,
                    mt_cc_curr,
                    cost_curr,
...
def build_comments_for_controllable(movements: pd.DataFrame) -> pd.DataFrame:
    ...
    Filter movements where ContrSide == "Controllable".
    Round in final text.
    """
    if movements.empty:
        return pd.DataFrame(columns=["MT_CC", "Comments"])

    # Filter to controllable side
    ...
    # Transfer in details: From_MT_CC breakdown
    tin = (
    ...
    # Transfer out details: To_MT_CC breakdown
    tout = (
...
def add_totals_rows(df: pd.DataFrame,
    ...
    if rows:
        final = pd.concat(rows + [grand], ignore_index=True)
    else:
        final = grand
...
def main():
    ...
    print("Building controllable comments...")
    ...
    # Replace NaN comments with empty string
    ...
    # Rename HC columns to date labels
